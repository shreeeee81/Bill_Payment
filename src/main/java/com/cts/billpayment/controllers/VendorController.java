...
import com.cts.billpayment.service.Vendorservice;
...
public class VendorController {
	...
	public String vendorlogin(@Valid @ModelAttribute("vendor") Vendorlogin vl, BindingResult result, Model model,
			HttpSession session) {
		if (result.hasErrors()) {
			return "vendorlogin";
		} else {
	...
	public String signup(@Valid @ModelAttribute("venreg") Vendor vendor,BindingResult result, @ModelAttribute("securityque") Security sec,
			 Model model) {
		...
		if (result.hasErrors()) {
			...
			if (res == 0) {
				...
				model.addAttribute("message",
						vendor.getUsername().toUpperCase() + " Congrats your registration with BILL PAYMENT is successfull");
	...
	public String getUserid(@RequestParam("mobile") String contactNumber,
			@RequestParam("secretquestion") String question, @RequestParam("answer") String answer, Model model) {
	...
	public String getPassword(@RequestParam("username") String userid, @RequestParam("secretquestion") String que,
		...
		System.out.println(sc);
		if (sc == null) {
			model.addAttribute("message", "Your Username is not registered with us! ");
			return "ForgotPassword";
		}
...
//String userid = (String)session.getAttribute("username");
		...
		if (pwd.equals(cpwd)) {
			...
			if (status == true) {
				...
				model.addAttribute("message", "Password has been set successfully");
				return "ResetPassword";
			} else {
	...
	public String updateVendor(@ModelAttribute("edit") Vendor vendor, Model model) {
		int status = vs.saveOrUpdate(vendor);
		if (status == 1) {
