...
import com.cts.billpayment.service.Adminservice;
...
public class AdminController {
	...
	public String loginverify(@Valid @ModelAttribute("admin") Aminlogin al, BindingResult result, Model model,
			HttpSession session) {
		if (result.hasErrors()) {
			return "adminlogin";
		} else {
	...
	@GetMapping("/tranasction2")
	public String generatereport2(Model model) {

		List<telephone> list = as.getTelephonebills();
		List<telephone> l1 = new ArrayList<telephone>();

		for (telephone b : list) {
			if (b.getTransactionstatus().equalsIgnoreCase("success")) {
				l1.add(b);
			}
	...
	@GetMapping("/fail2")
	public String generatefailreport2(Model model) {
		List<telephone> list = as.getTelephonebills();
		List<telephone> l2 = new ArrayList<telephone>();
		for (telephone b : list) {
